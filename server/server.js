...
const shouldGetUpdatedVerification = require("./utils")
  .shouldGetUpdatedVerification;

const VerificationIntent = StripeResource.extend({
  create: StripeResource.method({
    headers: {
      "Stripe-Version": "2019-12-03;identity_beta=v3"
    },
    method: "POST",
  ...
  verificationIntent.create(
    {
      return_url:
        req.get("origin") +
        "/next-step?verification_intent_id={VERIFICATION_INTENT_ID}",
...
app.post("/webhook", async (req, res) => {
  ...
  switch (eventType) {
    case "identity.verification_intent.created":
      console.log("\nVerificationIntent created", data);
      break;
  ...
  socket.on("init", data => {
    ...
    try {
      ...
      shouldCallApi = cache.shouldUpdateValue(
        ...
        shouldGetUpdatedVerification(latestVerification)
      );
    } catch (error) {
      shouldCallApi = true;
    }
